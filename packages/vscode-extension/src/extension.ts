...
function configForFix(base: Record<string, unknown>): Record<string, unknown> {
  return {
    ...base,
    sortClasses: true,
    removeRedundant: true,
...
function outputToDiagnostics(
  ...
  if (total === 0) return diagnostics;

  const range = new vscode.Range(0, 0, 0, 0);
  const parts: string[] = [];
  if (conflictCount > 0) parts.push(`${conflictCount} conflict(s)`);
  if (redundancyCount > 0) parts.push(`${redundancyCount} redundant`);
  if (suggestionCount > 0) parts.push(`${suggestionCount} suggestion(s)`);
  diagnostics.push(
    new vscode.Diagnostic(
      range,
      `Tailwind Architect: ${parts.join(", ")}. Run "Tailwind Architect: Fix Classes" to fix.`,
  ...
  for (const node of classNodes) {
    ...
    diagnostics.push(
      new vscode.Diagnostic(
        ...
        `Tailwind Architect: ${parts.join(", ")}. Run "Fix Classes" to fix.`,
        vscode.DiagnosticSeverity.Information
      )
    );
  }
...
async function fixDocument(editor: vscode.TextEditor): Promise<void> {
  ...
  try {
    ...
    if (collection) collection.delete(doc.uri);
  } catch (err) {
    vscode.window.showErrorMessage(
...
function getDiagnosticCollection(): vscode.DiagnosticCollection | null {
